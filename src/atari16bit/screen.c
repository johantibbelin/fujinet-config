...
{
  screen_gotoxy(0,0); printf(message);
}

void screen_error(const char *c)
{
  screen_clear_status();
  screen_gotoxy(0,STATUS_BAR + 1); printf("%-40s",c);
...
{
  screen_clrscr();
  screen_gotoxy(9,0); printf("WELCOME TO #FUJINET!");
  screen_gotoxy(0,2); printf("MAC Address: %02X:%02X:%02X:%02X:%02X:%02X",
    ac->macAddress[0],
    ac->macAddress[1],
    ac->macAddress[2],
    ac->macAddress[3],
    ac->macAddress[4],
    ac->macAddress[5]);
  screen_gotoxy(7,STATUS_BAR); printf("SCANNING FOR NETWORKS...");
}
...
void screen_set_wifi_display_ssid(char n, SSIDInfo *s)
{
  ...
  screen_gotoxy(2,n+4); printf("%s",ds);
  screen_gotoxy(36,n+4); printf("%s",meter);
...
void screen_set_wifi_select_network(unsigned char nn)
{
  screen_clear_status();
  bar_set(4,1,nn,0);
  screen_gotoxy(10,STATUS_BAR); printf("FOUND %d NETWORKS.\r\n", nn);
  screen_gotoxy(0, STATUS_BAR + 1);
...
{
  screen_clear_status();
  screen_gotoxy(0,STATUS_BAR); printf("ENTER NAME OF HIDDDEN NETWORK");
}
...
//  ostype = get_ostype() & 0xF0;
  ...
  screen_clear_status();
  screen_gotoxy(0,STATUS_BAR); printf("ENTER NET PASSWORD AND PRESS [RETURN]");
//  if (ostype == APPLE_II)
//  {
//    screen_gotoxy(0, STATUS_BAR - 1); printf("USE [ESC] TO SWITCH TO UPPER/LOWER CASE");
//  }
//  screen_gotoxy(0, STATUS_BAR + 1); printf(']');
}

void screen_connect_wifi(NetConfig *nc)
{
  screen_clear_status();
  screen_gotoxy(0,STATUS_BAR); printf("CONNECTING TO NETWORK: %s",nc->ssid);
}

void screen_destination_host_slot(char *h, char *p)
{
  screen_clrscr();
  screen_gotoxy(0, 12); printf("COPY FROM HOST SLOT");
  chlinexy(0,13,40);
  screen_gotoxy(0, 14); printf("%-32s", h);
  screen_gotoxy(0, 15); printf("%-128s", p);
}

void screen_destination_host_slot_choose(void)
{
  screen_gotoxy(0, 0);
  printf("COPY TO HOST SLOT");
  chlinexy(0,1,40);
...
void screen_hosts_and_devices_device_slots(unsigned char y, DeviceSlot *d, bool *e)
{
  ...
  for (i=0;i<4;i++)
    {
      screen_gotoxy(2,i+y); printf("%d %s",i+1,screen_hosts_and_devices_device_slot(d[i].hostSlot,e[i],d[i].file));
    }
}
...
void screen_hosts_and_devices(HostSlot *h, DeviceSlot *d, bool *e)
{
  ...
  screen_clrscr();
  screen_gotoxy(0,0);  printf("%40s",hl); // screen_inverse(0);
  chlinexy(0,0,40 - sizeof(hl));

  screen_gotoxy(0,10); printf("%40s",ds); // screen_inverse(10);
  chlinexy(0,10,40 - sizeof(ds));

  for (i=0;i<8;i++)
  {
    screen_gotoxy(2,i+1); printf("%d %s",i+1,screen_hosts_and_devices_slot(h[i]));
  }
...
void screen_hosts_and_devices_hosts(void)
{
  bar_set(1,1,8,0);
  screen_clear_status();
  // screen_gotoxy(0,STATUS_BAR); printf("[C]ONFIG  [E]DIT SLOT  [ESC]BOOT\r\n[1-8]HOST SLOT  [RETURN]SELECT SLOT\r\n[TAB] DEVICE SLOTS");
  screen_gotoxy(0,STATUS_BAR);
  ...
  //printf("[1-8]SLOT  [E]DIT  [RETURN]SELECT FILES\r\n [C]ONFIG  [TAB]DRIVE SLOTS  [ESC]BOOT");
}
...
void screen_hosts_and_devices_host_slots(HostSlot *h)
{
  ...
  for (i=0;i<8;i++)
    {
      screen_gotoxy(0,i+1); printf("%d %-32s",i+1,screen_hosts_and_devices_slot(h[i]));
    }
}
...
{
  screen_clear_status();
  screen_gotoxy(0,STATUS_BAR); printf("EDIT THE HOST NAME FOR SLOT %d\r\nPRESS [RETURN] WHEN DONE.",i);
}

void screen_perform_copy(char *sh, char *p, char *dh, char *dp)
{
  screen_clrscr();
  screen_gotoxy(0,0); printf("%32s","COPYING FILE FROM:");
  screen_gotoxy(0,1); printf("%32s",sh);
  screen_gotoxy(0,2); printf("%-128s",p);
  screen_gotoxy(0,6); printf("%32s",dh);
  screen_gotoxy(0,7); printf("%-128s",dp);
}
...
void screen_show_info(bool printerEnabled, AdapterConfig* ac)
{
  ...
  screen_gotoxy(4,5);
  printf("F U J I N E T      C O N F I G");
  screen_gotoxy(0,8);
  printf("%10s%s\r\n","SSID: ",ac->ssid);
  ...
  printf("   PRESS ANY KEY TO RETURN TO HOSTS\r\n");
  printf("      FUJINET PRINTER %s",(printerEnabled) ? "ENABLED" : "DISABLED");
  }

void screen_select_file(void)
{
  screen_clear_status();
  screen_gotoxy(0,STATUS_BAR); printf("OPENING...");
}
...
void screen_select_file_display(char *p, char *f)
{
  ...
  // Update content area
  screen_gotoxy(0,0); printf("%-40s",selected_host_name);
  screen_gotoxy(3,1);
  if (f[0]==0x00)
    printf("%-40s",p);
  else
...
{
  screen_clear_status();
  screen_gotoxy(0,STATUS_BAR); printf("ENTER A WILDCARD FILTER.\r\n E.G. *Apple*");
}
...
void screen_select_slot_eject(unsigned char ds)
{
  ...
  screen_gotoxy(4,1+ds); printf("EMPTY");
  bar_jump(bar_get());
}
...
void screen_hosts_and_devices_eject(unsigned char ds)
{
  ...
  screen_gotoxy(4,13+ds); printf("EMPTY");
  bar_jump(bar_get());
}
